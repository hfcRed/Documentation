...
Query data from any of the available NSO microservices. Currently available services are:
...
<Tabs>
  <TabItem label="200">
    ...
    <Tabs syncKey="example">
      <TabItem label="Example">
        <DynamicCode language="json" name="body" disabled>
          ```
          {
            "error": false,
            "data": [
...
* **0/7 (getting user access token)**
  ...
  This is the most common step you will encounter. It means that the session token you provided is invalid or expired. You should generate a new session token and try again.
