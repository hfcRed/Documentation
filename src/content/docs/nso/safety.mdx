...
Before obtaining a users session token, you must inform them of the use of this API and present the following policy and disclaimer to them, either directly in text or via a link to this page.

## Privacy Policy

```
This API does not store any private or identifying information.
For more details on the information that is stored, visit https://docs.hfcred.dev/nso/storage

This API will occasionally send a User ID to a third-party service as part of the authentication process.  
More information about this service can be found https://github.com/imink-app/f-API
...
For more information about the safety of this API, visit https://docs.hfcred.dev/nso/safety#about-safety
```
...
It is extremely unlikely that Nintendo will take action against any account for interfacing with their servers. Here are some points to consider:
    ...
    * Nintendo has only ever taken action against one application and its users in 2023 because it was putting heavy load on the NSO servers.
    ...
    * This API has very strict rate limits to make it impossible to generate more requests than you would when using the NSO app itself.
        * The rate limits can be found on the [Query Service](https://docs.hfcred.dev/nso/service/query#rate-limiting) page.
