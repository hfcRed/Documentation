...
In order to query the NSO servers, the API first needs to generate a Gameweb Token (gToken) and service-specific parameters.  
...
No personal or identifying information is ever stored.
...
Using the session token, the API generates the necessary gToken and parameters for the service you want to query.
